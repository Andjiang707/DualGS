...
def write_ply_with_attributes_plyfile(vertex_matrix, output_file, attributes_to_keep):
    n, k = vertex_matrix.shape
    dtype = [(attr, 'f4') for attr in attributes_to_keep]
    structured_array = np.empty(n, dtype=dtype)

    for i, attr in enumerate(attributes_to_keep):
        structured_array[attr] = vertex_matrix[:, i]
