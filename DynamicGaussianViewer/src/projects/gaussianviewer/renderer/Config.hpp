...
namespace sibr {
	...
	struct GaussianAppArgs :
		...
		RequiredArg<std::string> dataPathShort = { "d", "Path to the dataset root"};
		RequiredArg<std::string> start = { "start", "Start frame index" };
		RequiredArg<std::string> end = { "end", "End frame index" };
		Arg<std::string> step = { "step", "1", "Step interval between frame indices to be loaded" };
