...
sibr::GaussianView::GaussianView(const sibr::BasicIBRScene::Ptr & ibrScene, uint render_w, uint render_h, const char* file, bool* messageRead, int sh_degree, bool white_bg, bool useInterop, int device, int start, int end, int step, std::string datapath) :
	_scene(ibrScene),
	...
	sibr::ViewBase(render_w, render_h)
{
	...
	// std::string folder = "C:/Users/vrlab/Desktop/guochch/interpolate/our/dataset/ckt/";
	// std::string folder = "C:/Users/vrlab/Desktop/guochch/interpolate/our/output/boxing8000/k64/";

	int cnt = 0;

	for (int i = frame_st; i < frame_ed; i+=frame_step)
	{
		std::vector<Pos> pos;
		std::vector<Rot> rot;
		...
		std::vector<SHs<3>> shs;
		std::string file = folder + "point_cloud_" + std::to_string(i) + ".ply";
		std::cout<< file << std::endl;

		if (sh_degree == 0)
		{
			count = loadPly<0>(file.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
		}
		else if (sh_degree == 1)
		{
			count = loadPly<1>(file.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
		}
		else if (sh_degree == 2)
		{
			count = loadPly<2>(file.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
		}
		else if (sh_degree == 3)
		{
			count = loadPly<3>(file.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
		}		
		int P = count;
		// Allocate and fill the GPU data
		P_array[cnt] = P;
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&pos_cuda_array[cnt], sizeof(Pos) * P));
		CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(pos_cuda_array[cnt], pos.data(), sizeof(Pos) * P, cudaMemcpyHostToDevice));
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&rot_cuda_array[cnt], sizeof(Rot) * P));
		CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(rot_cuda_array[cnt], rot.data(), sizeof(Rot) * P, cudaMemcpyHostToDevice));
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&scale_cuda_array[cnt], sizeof(Scale) * P));
		CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(scale_cuda_array[cnt], scale.data(), sizeof(Scale) * P, cudaMemcpyHostToDevice));
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&opacity_cuda_array[cnt], sizeof(float) * P));
		CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(opacity_cuda_array[cnt], opacity.data(), sizeof(float) * P, cudaMemcpyHostToDevice));
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&shs_cuda_array[cnt], sizeof(SHs<3>) * P));
		CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(shs_cuda_array[cnt], shs.data(), sizeof(SHs<3>) * P, cudaMemcpyHostToDevice));
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&rect_cuda_array[cnt], 2 * P * sizeof(int)));
		gData_array[cnt] = new GaussianData(P, 
		(float*)pos.data(),
		(float*)rot.data(),
		(float*)scale.data(),
		opacity.data(),
...
void sibr::GaussianView::setFolder(const std::string & folder)
{
	this->folder = folder;
}
...
void sibr::GaussianView::onGUI()
{
	...
	ImGui::SliderInt("Frame Id", &frame_id, 0, (frame_ed - frame_st + frame_step - 1) / frame_step -1 );
	
	if (ImGui::Button("prev")) {
		frame_id = frame_id - 1 + ((frame_ed - frame_st + frame_step - 1) / frame_step );
		frame_id %= ((frame_ed - frame_st + frame_step - 1) / frame_step );
	}
	...
	ImGui::Checkbox("Play", &_multi_view_play);
