...
namespace sibr { 
	...
	public:
		 ...
		 */
		GaussianView(const sibr::BasicIBRScene::Ptr& ibrScene, uint render_w, uint render_h, const char* file, bool* message_read, int sh_degree, bool white_bg = false, bool useInterop = true, int device=0, int start=0, int end=1, int step=1, std::string datapath="");
		int frame_st = 1800;
		int frame_ed = 2000;
		std::string folder = "";
	...
	protected:
		...
		int P_array[500];
		float* pos_cuda_array[500];
		float* rot_cuda_array[500];
		float* scale_cuda_array[500];
		float* opacity_cuda_array[500];
		float* shs_cuda_array[500];
		int* rect_cuda_array[500];
