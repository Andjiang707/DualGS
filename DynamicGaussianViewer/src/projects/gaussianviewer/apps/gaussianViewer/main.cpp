...
int main(int ac, char** av) 
{
	...
	std::cout<<myArgs.step.get();
	std::cout<<myArgs.step.get();
	std::cout<<myArgs.step.get();
	std::cout<<myArgs.step.get();
	std::cout<<myArgs.step.get();
	// Create the ULR view.
	GaussianView::Ptr	gaussianView(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfile.c_str(), &messageRead, sh_degree, white_background, !myArgs.noInterop, device, std::stoi(myArgs.start.get()), std::stoi(myArgs.end.get()), std::stoi(myArgs.step.get()), myArgs.dataPathShort.get()));
	// gaussianView->frame_st = std::stoi(myArgs.start.get());
	// gaussianView->frame_ed = std::stoi(myArgs.end.get());
	// SIBR_LOG <<  "nmsl:               " << myArgs.dataPathShort.get() << std::endl;
	gaussianView->setFolder(myArgs.dataPathShort.get());
	// gaussianView->folder = myArgs.dataPathShort.get().c_str();
	// Raycaster.
